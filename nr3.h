...
public:
	...
	void addVec (int row, const T & value) ; //Ajout le 26/11/2021 par affane vir (l 235)
	~NRvector();
};
...
//Ajout 26/11/2021 par Affane
template <class T>
void NRvector<T>::addVec (int row, const T & value) {
	if (row  != nn ) v[row] = value ;
}
//Fin ajout
template <class T>
NRvector<T>::~NRvector()
...
public:
	...
	void add (int row, int col, const T & value) ;//Ajouté par Eugène Capo-Chichi pour charger la matrice (voir l-409)
	void print ();//Ajouté par Eugène Capo-Chichi pour faire de l'affichage (voir l 420)
	~NRmatrix();
};
